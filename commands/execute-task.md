...
5. **Make the worktree's gitignored dependencies (package manager installs, build output, `.env`) usable.** A fresh `git worktree` only contains tracked files — `vendor/`, `node_modules/`, `.venv/`, `target/`, local `.env` files, etc. do **not** exist in it. Get the worktree to a runnable state the cheapest way that's still correct:
   - If a full, independent dependency install (`composer install`, `npm install`, `pip install -r ...`, etc.) is fast enough and the project's lockfile guarantees a reproducible install, just run it inside `WORKTREE_PATH`. This is the simplest option and needs no further care.
   - If a fresh install is slow and the original checkout already has the dependencies installed, it's faster to **share** them instead of reinstalling — but a single `ln -s <original>/vendor vendor` (or `node_modules`) is not always enough:
     - Some package managers bake **absolute paths** into a generated autoloader/manifest at install time (PHP's Composer with `optimize-autoloader` is a known case). Symlinking the whole dependency directory then reuses the original checkout's absolute paths for the *project's own* source code — meaning the worktree's own files never actually get loaded. Symptom: code behaves as if your new files don't exist, even though the console can `require` them directly. Fix: regenerate just the project-local part of the autoloader/manifest from inside the worktree (e.g. `composer dump-autoload`) — reusing the shared third-party packages (no network needed) while correctly pointing the project's own namespaces at the worktree.
     - Symlinking whole-directory can also break tools that call `realpath()` on their own binary to find "their" project root (a common defensive pattern for CLI tools installed via a `bin/` symlink convention) — if the *whole package directory* housing that binary is itself a symlink into the original checkout, `realpath()` collapses through it and the tool concludes it's running from the original checkout, not the worktree. Symptom: the tool behaves correctly by every other measure, but mysteriously can't see the worktree's own files (e.g. picks up the wrong test root, wrong config). Fix: copy that one specific package for real (not symlinked) into the worktree instead of symlinking it, while leaving the rest of the dependency tree symlinked.
     - After regenerating anything, re-run any post-install hooks the framework relies on (e.g. Laravel's `package:discover` writes `bootstrap/cache/packages.php`/`services.php`) and make sure those cache files were written using the worktree's own path, not the original checkout's — delete and let them regenerate if in doubt.
   - If tests/tooling need to run **outside the project's normal container/VM** (e.g. because that container is already bound to the original checkout's directory and can't see the worktree), and the project's `.env`/config references container-only service hostnames (a Docker Compose service name like `mysql` or `redis` that only resolves inside that network), point the worktree's own `.env` at whatever host-reachable address reaches the same service instead (e.g. `127.0.0.1` on the port the container already publishes) — but only in the worktree's copy of the config, never the original checkout's.
   - Do not commit any of this: symlinks, `.env`, and regenerated caches for the worktree are all local scaffolding, not part of the ticket's diff.
   - Verify once, briefly, that the setup actually works (e.g. confirm a newly-added class from Kirk's future changes will resolve) before moving on — cheaper to catch a broken worktree now than after Bender reports confusing failures later.

6. Report the base branch, feature branch, and `WORKTREE_PATH` to the user before continuing.
